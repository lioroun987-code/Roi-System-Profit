...
export default function AnalyticsPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center h-64">
      <div className="animate-spin w-8 h-8 border-2 border-[var(--color-brand-start)] border-t-transparent rounded-full" />
    </div>
  )
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={label} className="rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-surface)] p-4">
            <p className="text-[var(--color-text-secondary)] text-sm">{label}</p>
            <p className={`text-xl font-bold mt-1 ${green ? 'text-[var(--color-success)]' : 'text-white'}`}>{value}</p>
          </div>
        ))}
      </div>
            ...
            <div className="flex gap-2">
              {(['area', 'bar'] as const).map(type => (
                <button key={type} onClick={() => setChartType(type)}
                  className={`text-xs px-3 py-1 rounded-full transition-colors ${chartType === type ? 'brand-gradient text-white' : 'text-[var(--color-text-secondary)] hover:text-white'}`}>
                  {type === 'area' ? 'קו' : 'עמודות'}
                </button>
              ))}
            </div>

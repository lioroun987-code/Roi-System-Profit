  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 border border-white/10 rounded-xl p-6">
        ...
        {tab === 'cost-rules' && business && (() => {
          const dr = business.discountRules ?? {}
          const rules: any[] = dr.costRules ?? []
          return (
            <div className="space-y-4">
              <div>
                <h3 className="text-white font-semibold mb-1">כללי עלות מהסוכן</h3>
                <p className="text-sm" style={{ color: '#6B7280' }}>
                  כללים שמשפיעים על חישוב העלות — נוצרו דרך עוזר ה-AI או ידנית
                </p>
              </div>

              {rules.length === 0 ? (
                <div className="rounded-xl p-8 text-center" style={{ background: '#13161F', border: '1px solid #1E2130' }}>
                  <p className="text-white font-medium mb-2">אין כללי עלות מוגדרים</p>
                  <p className="text-sm mb-4" style={{ color: '#4A5174' }}>
                    פתח את עוזר ה-AI ✨ ותאר כללי הנחה מהסוכן
                  </p>
                  <p className="text-xs px-4 py-2 rounded-xl inline-block" style={{ background: '#0D1A2A', color: '#60A5FA' }}>
                    לדוגמה: "כשקונים 2+ דילים, הסוכן מוריד לי $1.80 לכל דיל"
                  </p>
                </div>
              ) : (
                <div className="space-y-3">
                  {rules.map((rule: any, idx: number) => (
                    <div key={rule.id} className="rounded-xl p-4 flex items-start justify-between gap-4"
                      style={{ background: rule.active ? '#13161F' : '#0D0F14', border: `1px solid ${rule.active ? '#1E2130' : '#1A1D2A'}`, opacity: rule.active ? 1 : 0.5 }}>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <span className="text-white font-medium text-sm">{rule.name}</span>
                          <span className="text-xs px-2 py-0.5 rounded-full"
                            style={{ background: rule.active ? '#0D2818' : '#1A1D2A', color: rule.active ? '#22C55E' : '#4A5174' }}>
                            {rule.active ? 'פעיל' : 'מושבת'}
                          </span>
                        </div>
                        <p className="text-xs" style={{ color: '#4A5174' }}>
                          {rule.condition.type === 'quantity_of_type' && `כשכמות ${rule.condition.productType} ${rule.condition.operator} ${rule.condition.value}`}
                          {rule.condition.type === 'quantity_same_product' && `כשכמות אותו מוצר ${rule.condition.operator} ${rule.condition.value}`}
                          {rule.condition.type === 'total_items' && `כשסך פריטים ${rule.condition.operator} ${rule.condition.value}`}
                          {' → '}
                          {rule.effect.type === 'reduce_cost_per_unit' && `הפחת $${rule.effect.value} לכל יחידה`}
                          {rule.effect.type === 'set_cost_per_unit' && `קבע עלות יחידה ל-$${rule.effect.value}`}
                          {rule.effect.type === 'percent_off_total' && `${rule.effect.value}% הנחה על עלות`}
                        </p>
                        {rule.note && <p className="text-xs mt-1" style={{ color: '#374151' }}>{rule.note}</p>}
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          onClick={async () => {
                            ...
                            await save({ discountRules: { ...dr, costRules: updated } }, 'cost-rules')
                          }}
              ...
              <div className="rounded-xl p-4" style={{ background: '#0D1A2A', border: '1px solid #1E3A5F' }}>
                <p className="text-sm font-medium mb-1" style={{ color: '#60A5FA' }}>💡 איך מוסיפים כלל?</p>
                <p className="text-sm" style={{ color: '#4A5174' }}>
                  פתח את עוזר ה-AI ✨ (פינה שמאל תחתון) ותאר את הכלל בעברית.
                  לדוגמה: "כשקונים 2 דילים מאותו סוג, הסוכן מוריד $1.80 לכל דיל"
                </p>
              </div>
            </div>
          )
